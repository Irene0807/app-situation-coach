...
class UserRepository {
  ...
  Future<void> setSceneConversation(
      {required String journeyId,
      required String conversationId, // 'scene_conversation_01-01'
      required List<Message> messages}) async {
    await dbService.setDocument([
      'users',
      getCurrentUserId()!,
  ...
  // 要跑兩次 兩個journey各跑一次
  Future<JourneyStatus?> getJourneyStatus(String journeyId) async {
    Map<String, dynamic>? journey = await dbService
        .getDocument(['users', getCurrentUserId()!, 'journeys', journeyId]);
