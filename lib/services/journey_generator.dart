...
class JourneyGenerator {
  ...
  Future<String> generateJourneyPlan(String userInput) async {
    // 把使用者的原prompt轉乘plan
    final prompt = getJourneyPlanPrompt(userInput);
    final journeyPlan = geminiA.sendPrompt(prompt);
    return journeyPlan;
  }

  Future<String> generateJourneySchedule(String plan) async {
    // 透過經使用者修改過後的plan生成schedule
    final prompt = getJourneySchedulePrompt(plan);
    final journeySchedule = geminiA.sendPrompt(prompt);
